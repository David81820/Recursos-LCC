...
Isto é, uma função que dado um número x devolve-nos a função (x+). Por sua vez, quando a função (x+) recebe um número y ela devolve x+y.
...
É nos dito que `Aᴮ⁺ᶜ` e `Aᴮ x Aᶜ` são isómorficos ou seja, uma função que recebe ou B ou C e dá um A é a mesma coisa que ter duas funções, uma que só recebe Bs e outra que só recebe Cs.

<br>

**Como é que provamos isto?**

São nos dadas duas funções `join` e `unjoin` entre esses dois tipos e é nos dito para provar que são isomorfismos, ou seja: `join . unjoin = id` e `unjoin . join = id`.
<br>Como são funções de ordem superior, `join (f,g)` é pointwise: as funções são as variáveis se as definições estão em pointwise, temos que começar por introduzir variáveis no exercício.
...
**Que variáveis é que introduzo?**

Temos de introduzir dos dois lados (f,g).
<br>O `join` recebe um par de funções `Aᴮ x Aᶜ`
