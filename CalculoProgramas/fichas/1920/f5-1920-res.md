...
Relembrar
    ...
    in=[zero,succ]      # (sendo zero: 1 -> Nat)
    zero x = 0          # (note que zero é equivalente à função constante (const 0))
    succ: Nat -> Nat
    succ x = x+1

Relembrem o diagrama do combinador catamorfismo de naturais e a respectiva propriedade universal: `k=(|g|) <=> k.in=g.(id+k)`
...
É só usar o combinador `-|-`, por exemplo: `funcNat f = id -|- f`

Vamos agora a (|g|): `cataNat = cataNat g = g . funcNat (cataNat g) . outNat`

Codificando agora: `for b i =`, temos: `for b i=(| [(const i), b] |)` 
<br>Logo: `for b i = cataNat (either (const i) b)`

Estamos agora em condições de codificar:

    f = p2 . aux
    where aux = for (split (succ . p1) mul) (1,1)
    mul (x,y) = x*y
    
Vamos analisar com um diagrama.
<br>Qual o resultado de `aux 0` ?
...
**AVISO DO EDITOR:** infelizmente falta aqui uma última imagem que não consigo encontrar, peço desculpa pelo inconveniente.
...
E ainda que a função que soma todos os valores em folhas de árvores,
